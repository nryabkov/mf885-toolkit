/* MF885 Community 0.4.7-dev.18: explicit one-shot stock power operations. */
(function(w){
  'use strict';
  var app=w.MF885Community047Dev18,b=app.powerBridge,busy=false,consumed=false,appNonce=2;
  ...
  function powerFault(code,message){var error=Error(message);error.mfCode=code;return error}
  ...
  function apply(action){
    ...
    var token=b.begin('power','powerStatus');if(!token)return Promise.resolve(false);
    ...
    return get('/login.cgi','',true,epoch).then(function(reply){
      ...
      return get('/login.cgi?'+query,header,false,epoch);
    }).then(function(reply){
      ...
      return get('/xml_action.cgi?method=get&module=duster&file=status1',header,false,epoch);
    }).then(function(reply){
      if(!b.exactIdentity(parse(reply)))throw Error('The exact MF885 / Ver.D / 2.5.94 identity was not confirmed.');
      consumed=true;
      return get('/xml_action.cgi?method=get&module=duster&file='+(action==='reboot'?'reset':'poweroff'),header,false,epoch);
    }).then(function(reply){
      if(reply.body.trim()){
  ...
  w.MF885Community047Dev18Power={apply:apply,syncControls:syncControls};syncControls();
})(window);
