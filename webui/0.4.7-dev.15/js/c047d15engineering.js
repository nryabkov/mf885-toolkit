/* Community 0.4.7-dev.15: explicit Engineering control, serialized with other router actions. */
(function(w){
  'use strict';
  var core=w.MF885Community047Dev15, bridge=core&&core.engineeringBridge;
  ...
  function begin(owner){if(!bridge||!bridge.sessionPresent()||state.busy||!bridge.begin(owner,'engineeringStatus'))return false;state.busy=true;syncControls();return true}
  ...
  function read(){
    var owner='engineering-read',epoch=state.epoch;if(!begin(owner))return Promise.resolve(null);
    message('Reading the setting…');
    return get(owner,epoch).then(function(current){show(current);state.locked=false;message('Setting read. Choose Enabled or Disabled, then Apply.');return current}).catch(function(e){if(epoch===state.epoch){invalidate();message('The setting could not be read. Read again when the connection is available. ['+(e.mfCode||'E_ENGINEERING_READ')+']',true)}return null}).finally(function(){end(owner)});
  }
  function apply(value){
    ...
    if(!begin(owner))return Promise.resolve(null);message('Checking the current setting…');
    ...
    }).catch(function(e){if(epoch===state.epoch){invalidate();message((submitted?'The result is not confirmed. No change was sent again. Read the setting to check it.':'The setting could not be checked; nothing was applied.')+' ['+(e.mfCode||'E_ENGINEERING_CHECK')+']',true)}return null}).finally(function(){end(owner)});
  }
  ...
  w.MF885Community047Dev15Engineering={version:'0.4.7-dev.15',state:state,parseState:parseState,read:read,apply:apply,reset:reset,syncControls:syncControls,bind:bind};
