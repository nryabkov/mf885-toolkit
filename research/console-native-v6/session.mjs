/* Private c1 command session. Explicit sends, GET-only recovery, bounded RAM history. */
...
export function createConsoleSession({read,post,random=words=>globalThis.crypto.getRandomValues(words)}) {
  let busy=false,phase='unavailable',joined=null,current=null,action=null,history=[];
  const copy=a=>a?{...a,reply:a.reply?{...a.reply}:null}:null;
  const state=()=>({busy,phase,canSubmit:!busy&&phase==='ready',snapshot:current,action:copy(action),history:history.map(copy)});
    ...
    if(action){
      if(!c.initialized||c.nonce!==action.nonce){action.outcome='session-lost';action.reply=null;}
        ...
        if(action.kind==='u'){
          ...
          if(matches.length){action.reply={...matches[matches.length-1]};action.outcome='ussd-reply';}
        }
