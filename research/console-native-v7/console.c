...
/* Called from ARM scheduler/IRQ stubs; never use an unchecked raw arena. */
void cc_cpu_begin(void) {
    uint32_t saved=ENTER();uo_port_arena *a=arena();
    if(a)cpu_idle_begin(&a->cpu,cpu_native_read,0);
    LEAVE(saved);
}
void cc_cpu_end(void) {
    uint32_t saved=ENTER();uo_port_arena *a=arena();
    if(a)cpu_idle_end(&a->cpu,cpu_native_read,0);
    LEAVE(saved);
}
...
/* Optional cpu_v1 XML field alongside console_v1. When the field is absent the
 * old GET path is completely unchanged. When present, one retained CPU
 * snapshot is rendered into a fresh owned string. `cpu_snapshot` takes the
 * module's own short guard around the state copy and the optional pending
 * read; serialization happens OUTSIDE that guard. No division and no percentage
 * happen here, and no register is re-read after serialization. */
static int read_cpu(void *t) {
    void *node=LOOKUP(t,"cpu_v1");if(!node)return 0;
    char **slot=TEXT(node);if(!slot)return -1;
    char *out=NEW_BUFFER(CPU_WIRE_BYTES);if(!out)return -1;
    cpu_state frame;
    uint32_t saved=ENTER();uo_port_arena *a=arena();
    if(a)cpu_snapshot(&a->cpu,(cpu_read_fn)(uintptr_t)cpu_native_read,0,&frame);
    else cpu_init(&frame);
    LEAVE(saved);
    if(!cpu_serialize(&frame,out,CPU_WIRE_BYTES)){void *bad=out;DISCARD(&bad);return -1;}
    void *old=*slot;*slot=out;DISCARD(&old);return 0;
}
