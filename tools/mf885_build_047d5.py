#!/usr/bin/env python3
"""Build the pinned dev.5 source in an isolated offline Python process."""
import argparse
import json
import sys
...
def main(argv=None):
    ...
    sys.path[:0] = [str(ROOT / 'firmware/community-0.4.7-dev.5'), str(ROOT / 'tools')]
    ...
    try:
        ...
        report = candidate.comparator.publish_candidate(
            ...
            schema='mf885-047d5-public-build/v1',
            verification_schema='mf885-047d5-public-verify/v1',
            ...
            qualification={
                ...
                'reference_status': 'one-device Engineering read/enable/changing readings/disable and recovery proof; native unchanged from dev.4',
                'reference_date': '2026-09-07',
                'qualification_document': 'docs/ENGINEERING_047D5_RU.md',
        ...
        print('dev.5 offline build failed: ' + str(exc), file=sys.stderr)
        return 2
