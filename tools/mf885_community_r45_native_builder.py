#!/usr/bin/env python3
"""Build and verify the exact-golden R4.5 fixed64 forwarding firmware offline."""
...
ARTIFACT = "MF885_Community_0.4.5-community-r2-native-r19-cafe-r2.bin"
SCHEMA = "mf885-community-r45-fixed64-forward-build/v1"
VERIFICATION_SCHEMA = "mf885-community-r45-fixed64-forward-verification/v1"
LABEL = "R4.5"
...
QUALIFICATION = {
    ...
    "reason": "Offline ARMv5/Thumb-1 container with byte-wide packet access; hardware execution, forwarding and absolute egress TTL remain unproved.",
}


class CommunityR45NativeError(RuntimeError):
    pass
...
def _native_verification_conditions(native_report: dict[str, Any], safety: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    checks = [
        ...
        condition("native_helper_bytes", 132, component["bytes"]),
        ...
        condition("native_header_alignment", "Any byte alignment; packet accesses are byte-wide.", contract["header_alignment"]),
        condition("native_packet_store", "Three volatile byte stores at header+10, +11, then +8, before output when TTL differs; protocol is never stored.", contract["packet_store"]),
        condition("native_architecture", {key: value.decode("ascii") for key,value in native.TARGET.items()}, native_report["architecture"]),
...
def build_candidate(golden_raw: bytes, identity: inspector.IdentityMaterial) -> tuple[bytes, dict[str, Any]]:
    return comparator.build_candidate(golden_raw, identity, profile=PROFILE, schema=SCHEMA, label=LABEL, native_build=native.build_payload, error=CommunityR45NativeError)


def verify_candidate(golden_raw: bytes, candidate: bytes, identity: inspector.IdentityMaterial) -> dict[str, Any]:
    return comparator.verify_candidate(golden_raw, candidate, identity, profile=PROFILE, verification_schema=VERIFICATION_SCHEMA, label=LABEL, native_build=native.build_payload, native_conditions=_native_verification_conditions, error=CommunityR45NativeError)


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Build the full R4.5 firmware container offline; does not flash")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = comparator.publish_candidate(
            ...
            error=CommunityR45NativeError,
        )
        print(json.dumps(report, indent=2, sort_keys=True))
        return 0
    except (CommunityR45NativeError, shared.CommunityR30NativeError, r33_builder.CommunityR33NativeError, native.TtlR45PayloadError, thumb.NativeBuildError, inspector.InspectionError, webi.BuildError, stage.StageBuildError, OSError, lzma.LZMAError, TypeError, ValueError) as exc:
        print(f"R4.5 native build failed: {exc}", file=sys.stderr)
        return 2
