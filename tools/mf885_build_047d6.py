#!/usr/bin/env python3
"""Build the pinned dev.6 source in an isolated offline Python process."""
import argparse
import json
import sys
...
def main(argv=None):
    ...
    sys.path[:0] = [str(ROOT / 'firmware/community-0.4.7-dev.6'), str(ROOT / 'tools')]
    ...
    try:
        ...
        report = candidate.comparator.publish_candidate(
            ...
            schema='mf885-047d6-public-build/v1',
            verification_schema='mf885-047d6-public-verify/v1',
            ...
            qualification={
                ...
                'reference_status': 'one-device WAN/layout, reboot and operator-observed shutdown/physical-on/recovery proof; native unchanged from dev.5',
                'reference_date': '2026-09-07',
                'qualification_document': 'docs/WAN_POWER_047D6_RU.md',
        ...
        print('dev.6 offline build failed: ' + str(exc), file=sys.stderr)
        return 2
