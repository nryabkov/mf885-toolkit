...
class BuildVariantTests(unittest.TestCase):
    ...
    def test_dev5_and_dev6_use_isolated_offline_process_and_preserve_failure(self):
        for version, compact in (("0.4.7-dev.5", "047d5"), ("0.4.7-dev.6", "047d6")):
            with tempfile.TemporaryDirectory() as temporary, mock.patch.object(wrapper.subprocess, "run") as run:
                run.return_value.returncode = 2
                result = wrapper.main(["--variant", "community-" + version, "--output-dir", temporary, "--acknowledge-brick-risk"])
                self.assertEqual(result, 2)
                args = run.call_args.args[0]
                self.assertEqual(Path(args[2]).name, "mf885_build_" + compact + ".py")
                self.assertIn("--acknowledge-brick-risk", args)
                self.assertEqual(Path(args[args.index("--output") + 1]).name, "MF885-Community-" + version + "-base-2.5.94.bin")
            with tempfile.TemporaryDirectory() as temporary, mock.patch.object(wrapper.subprocess, "run") as run, contextlib.redirect_stderr(io.StringIO()):
                self.assertEqual(wrapper.main(["--variant", "community-" + version, "--output-dir", temporary]), 2)
                run.assert_not_called()
