...
import mf885_community_r44 as previous
import mf885_community_r45 as release
import mf885_community_r45_native_builder as builder
...
ARTIFACT_SHA256 = "a5c2dc109101156d3c707231d269968c6fa65ab80e86025f4af50a0b8c764583"


class R45SourceTests(unittest.TestCase):
    def test_profile_has_distinct_marker_and_truthful_fixed64_limits(self):
        profile = stage.STAGE_PROFILES[release.PROFILE]
        self.assertEqual(profile["marker"], b"MF885 Community R4.5 extension 0.4.5-community-r2")
        self.assertEqual(profile["patcher"], "community-r4.5")
        ...
        # The prior fixed64 profile remains unchanged.
    ...
    def test_wrapper_delegates_only_to_offline_r45_builder(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(builder, "main", return_value=17) as run:
            result = wrapper.main(["--variant", "community-r4.5", "--output-dir", tmp, "--acknowledge-brick-risk"])
...
class R45AssetTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_version_surface_and_old_paths_are_absent(self):
        ...
        self.assertIn(b'href="/r45.html"', self.replacements["www\\index.html"])
        self.assertIn(b'href="/r45.html"', self.replacements["www\\html\\adminApp.html"])
        joined = b"\n".join(self.additions.values())
        for expected in (release.MARKER, b'src="js/r45app.js"', b'href="css/r45ui.css"', b"Fixed TTL 64", b"There is no On/Off switch", b"A measurement on this router is still required"):
            self.assertTrue(expected in joined, expected)
        for forbidden in (b"modelGet('diagnostic'", b"file=diagnostic", b"file=ttl_set", b"<command>ttl</command>", b"setInterval", b"/r44.html", b"js/r44app.js"):
    ...
    def test_modified_panel_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(release.CommunityR45Error):
...
class R45ContainerTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_rebuild_matches_retained_image_and_all_native_bytes(self):
        ...
        self.assertEqual(hashlib.sha256(bytes(header) + self.image[inspector.HEADER_SIZE:]).hexdigest(), "7de178f4bdf7c6d382a8b767b6de668a1d81abb345675ba8367e9da663173b31")
        ...
        self.assertEqual(native["artifact"]["sha256"], "b10de9270439557ef4aa409b5c453bb5989c5d33f92322e60dc0794398f2d8f1")
        self.assertEqual(native["component"]["sha256"], "02ce2cf936f5265d3a0eb7e6f2450cd9e5c359d516c857609006dd07182aa62a")
    ...
    def test_changed_payload_is_rejected_by_full_verifier(self):
        ...
        with self.assertRaises((builder.CommunityR45NativeError, inspector.InspectionError)):
    ...
    def test_short_or_wrong_header_image_is_rejected(self):
        for image in (self.image[:100], bytes(1024)):
            with self.subTest(length=len(image)), self.assertRaises((builder.CommunityR45NativeError, builder.shared.CommunityR30NativeError, inspector.InspectionError, ValueError, struct.error)):
