...
define i32 @ttl_post_set(i32 %phase, ptr %context) #0 {
entry:
...
numeric.1.validate:
  ...
  br i1 %n1.valid, label %numeric.2.read, label %reject
...
numeric.2.validate:
  ...
  br i1 %n2.valid, label %numeric.3.read, label %reject
...
numeric.3.read:
  ...
  br i1 %numeric.three.valid, label %numeric.store.three, label %reject

numeric.store.three:
  br label %set.numeric

set.off:
  %state.off = inttoptr i32 110309888 to ptr
  store volatile i8 0, ptr %state.off, align 1
  %off.output = inttoptr i32 110309940 to ptr
  br label %publish
...
set.numeric:
  ...
  %state.numeric = inttoptr i32 110309888 to ptr
  store volatile i8 %numeric.byte, ptr %state.numeric, align 1
  %numeric.output = inttoptr i32 110310008 to ptr
  br label %publish

reject:
  %rejected.output = inttoptr i32 110309964 to ptr
  br label %publish

publish:
  %value = phi ptr [ %off.output, %set.off ], [ %numeric.output, %set.numeric ], [ %rejected.output, %reject ]
  %setter = inttoptr i32 104887259 to ptr
  %output.field = inttoptr i32 110309916 to ptr
  %unused = call i32 %setter(ptr %model, i32 0, ptr %output.field, ptr %value)
  br label %cleanup
