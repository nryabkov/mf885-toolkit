...
publish.numeric:
  %numeric.output = inttoptr i32 100668540 to ptr
  br label %publish

publish:
  %arg = phi ptr [ %off.value, %publish.off ], [ %digits, %publish.numeric ]
  %result = phi ptr [ %off.output, %publish.off ], [ %numeric.output, %publish.numeric ]
  ...
  %ttl.value = inttoptr i32 100668500 to ptr
  ...
  %u2 = call i32 %setter(ptr %model, i32 0, ptr %output.field, ptr %result)
  br label %done
