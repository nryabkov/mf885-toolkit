...
class Instructions(base.Machine):
    def step(self):
        ...
        kind=(op>>6)&15
        if op&0xfc00==0x4000 and kind in (3,12,13):
...
class ForwardMachine(Instructions):
    BODY_BYTES=152
    EXECUTABLE_BYTES=138
    OUTPUT_RETURN_OFFSET=136
    PACKET_WRITE_PLAN=((10,1),(11,1),(8,1))
    def __init__(self,helper,packet,*,word,**kwargs):
        super().__init__(helper,packet,**kwargs)
        self.map(RAM,struct.pack('<I',word),False,'state')
...
class CallbackMachine(Instructions):
    def __init__(self,kind,body,data,*,word=0,phase=None,command=b'ttl',argument=b'65',
        ...
        self.code=((self.entry,self.entry+(232 if kind=='set' else 128)),)
        assert len(body)==(248 if kind=='set' else 148)
        self.map(self.entry,body,False,'code')
        self.map(STATE,data,False,'names')
        self.map(RAM,struct.pack('<I',word),True,'state')
    ...
    def run(self):
        ...
        start,data,_,_=self.region(RAM,4)
