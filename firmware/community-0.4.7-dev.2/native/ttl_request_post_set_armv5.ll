...
; One aligned commit. Revision is diagnostic for serialized writes; this is
; not compare-and-swap or a uniqueness guarantee across concurrent writers.

; Parse this request tree only. Borrowed values are never freed.
; Accept exact ttl + off|1..255, then one aligned 32-bit word store. No property getter/setter.
...
commit:
  %target = phi i32 [ 0, %set.off ], [ %numeric.value, %set.numeric ]
  %state = inttoptr i32 118149684 to ptr
  %old = load volatile i32, ptr %state, align 4
  %old.revision = lshr i32 %old, 8
  %next = add i32 %old.revision, 1
  %masked = and i32 %next, 16777215
  %wrapped = icmp eq i32 %masked, 0
  %revision = select i1 %wrapped, i32 1, i32 %masked
  %upper = shl i32 %revision, 8
  %packed = or i32 %upper, %target
  store volatile i32 %packed, ptr %state, align 4
  br label %done
