"""Private exact-golden registration for the guided TTL editor."""
import sys
...
stage.STAGE_PROFILES[PROFILE] = {
    **stage.STAGE_PROFILES['0.4.5-community-r2'],
    ...
    'artifact': 'MF885-Community-0.4.7-dev.4-base-2.5.94-web-only.bin',
    'patcher': 'private-047d4',
    'safety': {
        **stage.STAGE_PROFILES['0.4.5-community-r2']['safety'],
        ...
        'ttlBrowserValues': ['64', 'off'], 'rawResponseBodiesLogged': False,
        'buildPinned': True,
...
stage.DERIVED_PATCHERS['private-047d4'] = (release, release.Error)
...
def build_candidate(golden, identity):
    return comparator.build_candidate(golden, identity, profile=PROFILE, schema='mf885-047d4-private-build/v1', label=PROFILE, native_build=native_payload.build_payload, error=release.Error)

def verify_candidate(golden, image, identity):
    return comparator.verify_candidate(golden, image, identity, profile=PROFILE, verification_schema='mf885-047d4-private-verify/v1', label=PROFILE, native_build=native_payload.build_payload, native_conditions=native_conditions, error=release.Error)
